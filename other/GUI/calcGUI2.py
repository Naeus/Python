...
class CalculatorFrame(tk.Frame):                                                #Class for Calculator frame
    def __init__(self, master=None):                                            #How we initialize CalculatorFrame, and what is done when it's initialized.
        ...
        tk.Grid.columnconfigure(self, 0, weight=1)                              #There's only one column

    def widget_resize_event(event, widgetObjects):                               #Event that handles fontsize of widget's text upon resizing
        fontSize = event.height // 3                                            #should be along the lines of min(height/3, width /char_length)
...
class NumPad(tk.Frame):
    ...
    def buttons(self, master, buttonsNum=10, size=(4, 2), fntSize=8, tabEnabled=False):
        buttons = list((tk.Button(master=master, text=str(num), width=size[0], height=size[1], font=fntSize, takefocus=tabEnabled)) for num in range(buttonsNum-1, -1, -1))
        negateButton = tk.Button(master=master, text=chr(177), width=size[0], height=size[1], font=fntSize, takefocus=tabEnabled)
        buttons.append(negateButton)
        pointButton = tk.Button(master=master, text=".", width=size[0], height=size[1], font=fntSize, takefocus=tabEnabled, command=lambda: pointButton.destroy())
        ...
        for i in range(buttonsNum+2):
            ...
            self.buttons[i].bind("<Configure>", lambda event, widgetObjects=self.buttons: CalculatorFrame.widget_resize_event(event, widgetObjects))
...
class EntryFrame(tk.Frame):
    def __init__(self, master=None):
        ...
        self.entry1 = tk.Entry(self, justify="right", font=("Default", 24, "normal"),
